...
//sending msg
  ...
  if(flight.equals(flight3)){
    ...
    console.log('flight 3.2');
    server.send(data,2222,info.address,function(error){
      if(error){
        console.log('!Data sent !!!');
        console.log(retrieve_flightNo(data));
        server.close();
  ...
  if(flight.equals(flight5)){
    console.log('flight 5');
    data=flight5;
    clientPort=info.port;
    clientAddress=info.address;
    console.log('info addres : '+info.address);
    server.send(data,2222,'103.94.135.134',function(error){
      if(error){
        console.log('!Data sent !!!');
        console.log(data);
        server.close();
  ...
  if(flight.equals(cloud)){
    data=createFlight6();
    server.send(data,clientPort,clientAddress,function(error){
      if(error){
        console.log('!Data sent !!!');
        console.log(retrieve_flightNo(data));
        server.close();
